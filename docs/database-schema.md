# Database Schema — Orbya SaaS

> Supabase (PostgreSQL) | Todas tabelas com RLS ativo
...
create table profiles (
  ...
  directfy_api_key text, -- encrypted
  calendly_url text,
...
create table interactions (
  ...
  resposta_lead text,
  agent_reasoning text, -- log de decisão do agente
  metadata jsonb default '{}',
  created_at timestamptz default now()
);
...
### agent_queue (fila de ações pendentes)
```sql
...
```sql
-- Pattern: user can only see/modify their own data
...
-- Repeat for: campaigns, cadencia_steps, interactions, agent_queue
-- cadencia_steps and interactions use join through campaign.user_id
```

## Functions

```sql
-- Update lead score and auto-move pipeline
...
-- Trigger: update campaign counters
create or replace function update_campaign_counters()
returns trigger as $$
begin
  update campaigns set
    total_enviados = (select count(*) from interactions where campaign_id = NEW.campaign_id and tipo = 'enviado'),
    total_respondidos = (select count(distinct lead_id) from interactions where campaign_id = NEW.campaign_id and tipo = 'respondido'),
    updated_at = now()
...
- `interactions` — for live timeline updates
- `leads` — for live score/status changes
- `agent_queue` — for monitoring agent activity
